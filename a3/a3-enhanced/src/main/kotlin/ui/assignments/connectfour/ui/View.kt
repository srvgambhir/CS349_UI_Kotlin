...
class View: Pane() {
    ...
    init {
        /**
         * Call function to create items in view
         */
        createView()
    ...
    private fun createView() {
         ...
         * Add Reset button
         */
        children.add(Button("Reset").apply {
            prefWidth = 150.0
            prefHeight = 50.0
            layoutX = 25.0
            layoutY = sceneHeight - 100.0
            font = Font.font(25.0)

            /**
             * On press, initialize new grid and reset values
             */
            setOnAction {
                Model.grid = Grid(Model.width, Model.height, Model.length)
                Model.onGameWin.value = Player.NONE
                Model.onGameDraw.value = false
                Model.onNextPlayer.value = Player.NONE
