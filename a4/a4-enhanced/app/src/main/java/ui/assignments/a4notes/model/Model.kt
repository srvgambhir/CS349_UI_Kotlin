...
class Model {
    ...
    fun updateNoteArchived(id: Int, archived: Boolean) {
        var updateNote: ModelNote? = null
        notes.find { it.id == id }.apply {
            this?.important = if (archived) false else this!!.important
            this?.archived = archived
            /**
             * If removing archived status, have to recreate the VMNote
             */
            if (!archived) {
                updateNote = this!!
            }
        ...
        /**
         * Remove the note from model, and add it back
         * Triggers the NOtesViewModel observer functions to recreate the VMNote
         */
        if (updateNote != null) {
            notes.remove(updateNote)
    ...
    /**
     * Function for updating display list to include archived notes
     */
    fun updateArchivedNotes() {
        val updateList = mutableListOf<Model.ModelNote>()
        notes.forEach {
            if (it.archived) {
                updateList.add(it)
            }
        ...
        /**
         * Have to go through, removing and adding back the archived notes
         * This recreates the VMNotes to actually display on screen
         */
        updateList.forEach {
            notes.remove(it)
            it.archived=false
            notes.add(it)
            it.archived = true
            it.notifyPropertyChanged(it.id)
        }
    }
}